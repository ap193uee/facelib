...
class FaceRecDlib(object):
    ...
    def face_encodings(self, face_image, raw_landmarks, num_samples=1):
        ...
        :param num_samples: How many times to re-sample the face when calculating encoding. Higher is more accurate, but slower (i.e. 100 is 100x slower)
        :return: A list of 128-dimensional face encodings (one for each face in the image)
        """
        return [np.array(self.face_encoder.compute_face_descriptor(face_image, raw_landmark_set, num_samples)) for raw_landmark_set in raw_landmarks]
